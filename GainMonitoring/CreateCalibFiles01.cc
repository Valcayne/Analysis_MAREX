...
/*
Programa que crea ficheros de calibración, teniendo en cuenta los cambios de ganancia
...
int main(int argc, char** argv) {
  ...
  if(argc!=8 && argc!=9 && argc!=10){
    cout<<" ########## Input has to be like: CreateCalibFiles01 [InpCalibDir] [OutCalibDir] [GainShiftsDir] [DetName] [DetID] [RunDescriptor] [RefCalibRunNumber] ([RunMin=0]) ([RunMax=1.e20]) ##########"<<endl;
    return 1;
  }
  ...
  char* RunDescriptor=argv[6]; if(string(argv[6])=="0"){RunDescriptor=0;}
  int RefCalibRunNumber=std::atoi(argv[7]);
  double RunMin=0,RunMax=1.e20;
  if(argc>8){RunMin=std::atoi(argv[8]);}
  if(argc>9){RunMax=std::atoi(argv[9]);}
  ...
  CalibrationManager* theRefRunCal=new CalibrationManager(InpCalibDir,RefCalibRunNumber);
  double p0_ref=0,p1_ref=0,p2_ref=0;
  ...
  if(check!=0){cout<<" ############# ERROR in "<<__FILE__<<", line "<<__LINE__<<" #############"<<endl; exit(1);}
  ...
  while(in1>>inCalibFname){
    ...
    if(thisRun>=RunMin && thisRun<=RunMax){
      CalibrationManager* theCalMan=new CalibrationManager(InpCalibDir,thisRun,0);
      double GainShift=-1;
      if(thisRun<MinRun_GS){GainShift=GS_min;}
      else if(thisRun>MaxRun_GS){GainShift=GS_max;}
      else{
	GainShift=gr1->Eval(thisRun);
      ...
      theCalMan->SetEneCalibParams(DetName,DetID,p0,p1,p2);
